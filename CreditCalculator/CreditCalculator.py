...
def n(principal, payment, interest):
    x = (interest * 0.01 / 12)
    fraction = (payment / (payment - x * principal))
    result_n = math.ceil(math.log(fraction, x + 1))
    years = result_n / 12
...
def a(principal, periods, interest):
    x = (interest * 0.01 / 12)
    num = pow(1 + x, periods)
    result_a = principal * ((x * num) / (num - 1))
...
def p(payment, periods, interest):
    x = (interest * 0.01 / 12)
    num = pow(1 + x, periods)
    result_p = math.floor(float(payment / ((x * num) / (num - 1))))
...
def diff(principal, periods, interest):
    ...
    for monthly in steps:
        monthly -= 1
        d = math.ceil(principal / periods) + x * ((principal * monthly) / periods)
        result += d
        month += 1
